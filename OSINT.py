# This is an OSINT tool by Twistens (@Twistens) to search for information about an email, phone number, or username.
# It checks whether the provided email is linked on popular websites, retrieves phone number details, 
# and searches for a username across multiple online platforms.

try:
	import requests,threading,phonenumbers, webbrowser
	from phonenumbers import carrier
	from time import sleep
except Exception as e:
	exit(e)

print('''This is an OSINT tool by Twistens (@Twistens) to search for information about an email, phone number, or username. \n It checks whether the provided email is linked on popular websites, retrieves phone number details, and searches for a username across multiple online platforms.''')

PRNT = threading.Lock()
r=requests.session()
def telegram_twistens(*a, **b):
	with PRNT:
		print(*a, **b)
def searched():
	global SNP,TIK,TWR,EML,INS,SCLD,NON,ACP,VIM,NEapi,DARK,eml
	telegram_twistens(f"[*] Email : {eml} \n\n{EML}\n{SNP}\n{TWR}\n{TIK}\n{INS}\n{SCLD}\n{NON}\n{ACP}\n{VIM}\n{NEapi}\n{DARK}")
# This tool was made by VOX / RUX.
# If you steal, edit, or try to sell this without permission — you're a leech.
# Don't touch what you didn’t build. Respect the creator.
def EMdark():
	global SNP,TIK,TWR,EML,INS,SCLD,NON,ACP,VIM,NEapi,DARK,eml
	try:
		...
		searched()
		webbrowser.open("https://t.me/twistens")
...
def number_search():
	...
	try:
		...
		#Communication rights reserved @Twistens
		pho=phonenumbers.parse('+'+phon)
		qtr = carrier.name_for_number(pho,'en')
		telegram_twistens(f'\n[+] phone : {nump}\n[+] country : {countr2}\n[+] ZIP code : {countr}\n[+] name : {name}\n[+] number type : {qtr}')
	except KeyError:
		telegram_twistens('[-] No leaked information found')
def All_users():
	username = input('[?] Enter username : ')
	try:file = open('Links.txt','r')
	except FileNotFoundError:exit('[-] Please make sure that the Links.txt file is in the same folder as the tool')
	telegram_twistens('\n      ========== started ==========\n')
	sleep(1)
	telegram_twistens(f'[+] Username : {username}')
	while True:
		...
		send = r.get(url,headers={'User-Agent': 'Mozilla/5.0 (@fesible) Gecko/20100101 Firefox/90.0'})
		if send.status_code == 200:telegram_twistens(f'[+] linked on {web_name}')
		elif send.status_code == 404:telegram_twistens(f'[-] Not linked on {web_name}')
		else:
			telegram_twistens(f'[!] search error [{web_name}]')
try:
	fesible=int(input('[?] Enter mode : '))
	if fesible==1:START_EML()
	elif fesible==2:number_search()
	elif fesible==3:All_users()
